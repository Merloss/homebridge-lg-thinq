...
export function createMqttReconnectState() {
    return { generation: 0, pending: false, attempt: 0, stopped: false };
}
export function stopMqttReconnect(state) {
    state.stopped = true;
    state.pending = false;
}
export function mqttReconnectDelayMs(attempt) {
...
export function wireMqttDeviceEvents(options) {
    const { device, logger, mqttServer, subscriptions, onMessage, reconnect, scheduleReconnect = (handler, delayMs) => {
        const timer = setTimeout(handler, delayMs);
        timer.unref?.();
        return timer;
    }, state = createMqttReconnectState(), } = options;
