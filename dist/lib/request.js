...
import { ManualProcessNeeded, ManualProcessNeededErrorCode, MonitorError, NotConnectedError, RateLimitError, retryAfterMs, TokenExpiredErrorCode, TokenExpiredError, NotConnectedErrorCodes, } from '../errors/index.js';
...
function errorWithCause(error, cause) {
    error.cause = cause;
    return error;
}
/**
...
function wrapAdapter(base) {
    ...
    wrapped[BASE_ADAPTER] = base;
    return wrapped;
}
/** Resolves the underlying adapter, unwrapping a previous wrap if present. */
function baseAdapterFor(candidate) {
    const previous = candidate?.[BASE_ADAPTER];
    if (previous) {
        return previous;
    }
 ...
 */
export function retryDelayFor(retryCount, err, random = Math.random) {
...
client.interceptors.response.use((response) => {
    ...
    if (!err.response) {
        ...
        throw errorWithCause(new RateLimitError(responseErrorMessage(err, 'LG ThinQ is rate limiting this account, increase refresh_interval.'), retryAfterMs(err.response.headers?.['retry-after'])), err);
    }
        ...
        throw errorWithCause(new ManualProcessNeeded('Please open the native LG App and sign in to your account to see what happened, ' +
            'maybe new agreement need your accept. Then try restarting Homebridge.'), err);
    }
    return Promise.reject(err);
});
