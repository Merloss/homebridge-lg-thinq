...
import { contactSensorStateValue, hasSnapshotKey, snapshotNumber, snapshotString, updateCharacteristicIfChanged, } from './helpers.js';
export function refrigeratorRefStateCommand(refState) {
    return {
        dataKey: null,
        dataValue: null,
        dataSetList: {
...
export function refrigeratorFeatureCommand(featureKey, value, onValue, offValue, tempUnit) {
    return refrigeratorRefStateCommand({
        [featureKey]: value ? onValue : offValue,
        tempUnit,
    });
}
export function refrigeratorTemperatureCommand(key, temp, tempUnit) {
    return refrigeratorRefStateCommand({
...
export default class Refrigerator extends BaseDevice {
    ...
    updateAccessoryCharacteristic(device) {
        ...
        if (this.Status.hasFeature('waterFilter') && this.serviceWaterFilter) {
            ...
            updateCharacteristicIfChanged(this.serviceWaterFilter, FilterChangeIndication, this.Status.waterFilterRemain < 5 ? FilterChangeIndication.CHANGE_FILTER : FilterChangeIndication.FILTER_OK);
        }
    }
