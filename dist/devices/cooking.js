...
export function durationFromSnapshot(snapshot, hourKey, minuteKey, secondKey) {
    return snapshotNumber(snapshot, hourKey) * 3600
        + snapshotNumber(snapshot, minuteKey) * 60
        + snapshotNumber(snapshot, secondKey);
}
export function cooktopOperationDuration(snapshot, cooktopNumber) {
    const prefix = `cooktop${cooktopNumber}`;
    return durationFromSnapshot(snapshot, `${prefix}OperationTimeHour`, `${prefix}OperationTimeMinute`, `${prefix}OperationTimeSecond`);
}
export function isFahrenheitValue(value) {
    return typeof value === 'string' && value.includes('FAH');
}
export function isFahrenheitUnit(snapshot, unitKey) {
    return isFahrenheitValue(snapshotString(snapshot, unitKey));
}
export function temperatureDisplayUnitsValue(snapshot, unitKey) {
    return isFahrenheitUnit(snapshot, unitKey) ? 1 : 0;
}
export function snapshotIncludes(snapshot, key, expected) {
    return snapshotString(snapshot, key).includes(expected);
}
export function isEnabledStatus(snapshot, key) {
    const status = snapshotString(snapshot, key);
    return status !== '' && !status.includes('DIS');
}
export function isInitialCookingState(snapshot, stateKey) {
    return snapshotIncludes(snapshot, stateKey, 'INITIAL');
}
export function hasActiveCookingState(snapshot, stateKey) {
    const state = snapshotString(snapshot, stateKey);
    return state !== '' && !state.includes('INITIAL');
}
export function hasCookingModeActive(snapshot, modeKey, idleValue) {
    const mode = snapshotString(snapshot, modeKey);
    return mode !== '' && !mode.includes(idleValue);
}
export function hasNonZeroSnapshotNumber(snapshot, key) {
    return snapshotNumber(snapshot, key) !== 0;
}
export function microwavePowerPercent(snapshot, key = 'LWOMGTPowerLevel') {
    return snapshotNumber(snapshot, key) * 10;
}
export function isCooktopActive(snapshot, cooktopNumber) {
    const state = snapshotString(snapshot, `cooktop${cooktopNumber}CooktopState`);
    return state !== '' && state !== 'INIT';
}
export function homeKitTemperatureFromSnapshot(snapshot, temperatureKey, unitKey) {
...
export function clampCookingTemperature(temperature, unit, fahrenheitRange, celsiusRange) {
    const [min, max] = isFahrenheitValue(unit) ? fahrenheitRange : celsiusRange;
    return Math.min(max, Math.max(min, temperature));
}
export function cookingTimerServiceUpdate(remainingDuration, targetDuration, isCooking, inUse) {
    return {
...
export function cookingAlarmServiceUpdate(timerDuration, inUse) {
    return {
...
function cookingTimerCommand(time, includeHours) {
    return {
        payload: {
            dataKey: null,
            dataValue: null,
...
export function ovenTimerCommand(time) {
    return cookingTimerCommand(time, true);
}
export function microwaveTimerCommand(time) {
    return cookingTimerCommand(time, false);
}
export function cookingStopCommand() {
    return {
        payload: {
            dataKey: null,
            dataValue: null,
...
export function prepareOvenCookCommand(command) {
    let next = {
        ...command,
    };
    ...
    if (next.ovenMode.includes('CONVECTION_BAKE') ||
        ...
        next.ovenSetTemperature = clampCookingTemperature(next.ovenSetTemperature, next.tempUnits, [300, 550], [150, 285]);
    }
    else if (next.ovenMode.includes('BAKE')) {
        next.ovenSetTemperature = clampCookingTemperature(next.ovenSetTemperature, next.tempUnits, [170, 550], [80, 285]);
    }
    else if (next.ovenMode.includes('AIR_SOUSVIDE')) {
        next.ovenSetTemperature = clampCookingTemperature(next.ovenSetTemperature, next.tempUnits, [100, 205], [38, 96]);
    }
...
export function ovenRemoteStartCommand(command) {
    return {
        payload: {
            dataKey: null,
            dataValue: null,
...
export function microwaveVentLampCommand(ventSpeed, lampLevel) {
    return {
        payload: {
            dataKey: null,
            dataValue: null,
...
export function prepareMicrowaveCookCommand(command, microwavePower) {
    let next = {
        ...command,
    ...
    if (isBakeOrOven) {
        next.ovenSetTemperature = clampCookingTemperature(next.ovenSetTemperature, next.tempUnits, [250, 450], [125, 230]);
        ...
        next.ovenSetTemperature = clampCookingTemperature(next.ovenSetTemperature, next.tempUnits, [100, 200], [38, 92]);
        next.subCookNumber = 0;
        next.microwavePower = '100';
        next.targetWeight = 0;
        next.weightUnits = 'LBS';
...
export function microwaveRemoteStartCommand(command) {
