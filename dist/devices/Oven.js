...
import { cooktopOperationDuration, cookingAlarmServiceUpdate, cookingStopCommand, cookingTimerServiceUpdate, durationFromSnapshot, hasActiveCookingState, hasCookingModeActive, hasNonZeroSnapshotNumber, homeKitTemperatureFromSnapshot, isCooktopActive, isEnabledStatus, isFahrenheitUnit, ovenRemoteStartCommand, ovenTimerCommand, prepareOvenCookCommand, temperatureDisplayUnitsValue, } from './cooking.js';
import { snapshotNumber, snapshotString, updateCharacteristicIfChanged, visibilityCharacteristicUpdate, } from './helpers.js';
var OvenState;
(function (OvenState) {
...
export default class Oven extends BaseDevice {
    ...
    constructor(platform, accessory, logger) {
        ...
        this.ovenDoorOpened.setCharacteristic(this.platform.Characteristic.ContactSensorState, isEnabledStatus(this.Status.data, 'upperDoorOpen') ? 1 : 0);
        ///Range Cooking
        ...
        this.remoteEnabled.setCharacteristic(this.platform.Characteristic.ContactSensorState, isEnabledStatus(this.Status.data, 'upperRemoteStart') ? 1 : 0);
        /////////Burners On
        ...
        this.burnersOnNumber.setCharacteristic(this.platform.Characteristic.CurrentAmbientLightLevel, burnerOnCounter < 1 ? 0.0001 : burnerOnCounter);
    ...
    remainTime() {
        return durationFromSnapshot(this.Status.data, 'upperRemainTimeHour', 'upperRemainTimeMinute', 'upperRemainTimeSecond');
    }
    ovenModeName() {
    ...
    oventTargetTime() {
        return durationFromSnapshot(this.Status.data, 'upperTargetTimeHour', 'upperTargetTimeMinute', 'upperTargetTimeSecond');
    }
    ovenTimerTime() {
        return durationFromSnapshot(this.Status.data, 'upperTimerHour', 'upperTimerMinute', 'upperTimerSecond');
    }
    tempCtoF(temp) {
    ...
    cooktopBurnerState(cooktopNumber, label) {
    ...
    updateAccessoryCharacteristic(device) {
        ...
        if (!this.pauseUpdate) {
            ...
            if (this.ovenTempControl.getCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits).value !== temperatureDisplayUnits) {
                this.ovenTempControl.updateCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits, temperatureDisplayUnits);
                this.probeTempControl.updateCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits, temperatureDisplayUnits);
            }
            ...
            ///////Timer Monitor
            const timerServiceUpdate = cookingTimerServiceUpdate(this.remainTime(), this.oventTargetTime(), this.ovenOnStatus(), Characteristic.InUse);
            ...
            /////Burners on
            const burnerOnCounter = snapshotNumber(this.Status.data, 'burnerOnCounter');
            this.burnersOnNumber.updateCharacteristic(this.platform.Characteristic.CurrentAmbientLightLevel, burnerOnCounter < 1 ? 0.0001 : burnerOnCounter);
