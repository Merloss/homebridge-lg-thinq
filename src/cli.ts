...
function redactValue(value: any, enabled: boolean): any {
...
async function connect(token: string | undefined, country: string, language: string) {
  if (!token) {
...
const options: { country: string; language: string; token?: string } = {
  country: 'US',
  language: 'en-US',
};
...
program
  ...
  .action(run(async (deviceId: string, cmdOptions: any) => {
    const api = await connect(options.token, options.country, options.language);
    const devices = devicesFromList(await api.getListDevices());
